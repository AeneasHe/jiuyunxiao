...
def get_mingzao():
    ...
    parser.add_argument(
        "-w", action="store_true", default=False, help="是否为女，默认为男"
    )
    parser.add_argument(
        "--version", action="version", version="%(prog)s 1.0 Rongzhong xu 2022 06 15"
    )
    ...
    if options.b:  # 直接输入八字
        ...
        for jd in jds:
            t = sxtwl.JD2DD(jd)
            print(
                "可能出生时间: python bazi.py -g %d %d %d %d :%d:%d"
                % (t.Y, t.M, t.D, t.h, t.m, round(t.s))
            )
