    ...
    if ming.lunar:
        ...
        for dayun in yun.getDaYun()[1:]:
                    ...
                    if gan_ == ming.gans[i]:
                        if abs(Zhi.index(zhi_) - Zhi.index(ming.zhis[i])) == 2:
                            jia = (
                                jia
                                + "  --夹："
                                + Zhi[(Zhi.index(zhi_) + Zhi.index(ming.zhis[i])) // 2]
                            )
                        if abs(Zhi.index(zhi_) - Zhi.index(ming.zhis[i])) == 10:
                            jia = (
                                jia
                                + "  --夹："
                                + Zhi[(Zhi.index(zhi_) + Zhi.index(ming.zhis[i])) % 12]
                            )
            ...
            out = "{1:<4d}{2:<5s}{3} {15} {14} {13}  {4}:{5}{8}{6:{0}<6s}{12}{7}{8}{9} - {10:{0}<10s} {11}".format(
                ...
                "",
                dayun.getGanZhi(),
                ten_deities[ming.me][gan_],
                gan_,
                check_gan(gan_, ming.gans),
                zhi_,
                yinyang(zhi_),
                ten_deities[ming.me][zhi_],
                zhi5_,
                zhi__,
                empty,
                fu,
            ...
            for liunian in dayun.getLiuNian():
                ...
                out = "{1:>3d} {2:<5d}{3} {15} {14} {13}  {4}:{5}{8}{6:{0}<6s}{12}{7}{8}{9} - {10:{0}<10s} {11}".format(
                    ...
                    liunian.getYear(),
                    gan2_ + zhi2_,
                    ten_deities[ming.me][gan2_],
                    gan2_,
                    check_gan(gan2_, gans2),
                    zhi2_,
                    yinyang(zhi2_),
                    ten_deities[ming.me][zhi2_],
                    zhi6_,
                    zhi__,
                    empty,
                    fu2,
                        ...
                        if gan2_ == gans2[i]:
                            ...
                            if abs(Zhi.index(zhi2_) - Zhi.index(zhis2[i])) == 2:
                                ...
                                jia = (
                                    jia
                                    + "  --夹："
                                    + Zhi[(Zhi.index(zhi2_) + Zhi.index(zhis2[i])) // 2]
                                )
                            if abs(Zhi.index(zhi2_) - Zhi.index(zhis2[i])) == 10:
                                ...
                                jia = (
                                    jia
                                    + "  --夹："
                                    + Zhi[(Zhi.index(zhi2_) + Zhi.index(zhis2[i])) % 12]
                                )

                            if (zhi1 + zhi2_ in gong_he) and (
                                gong_he[zhi1 + zhi2_] not in ming.zhis
                            ):
                ...
                if set("戌亥辰巳").issubset(all_zhis):
                    out = out + "  天罗地网：戌亥辰巳"
                if (
                    set("寅申巳亥").issubset(all_zhis)
                    and len(set("寅申巳亥") & set(ming.zhis)) == 2
                ):
                    out = out + "  四生：寅申巳亥"
                if (
                    set("子午卯酉").issubset(all_zhis)
                    and len(set("子午卯酉") & set(ming.zhis)) == 2
                ):
                    out = out + "  四败：子午卯酉"
                if (
                    set("辰戌丑未").issubset(all_zhis)
                    and len(set("辰戌丑未") & set(ming.zhis)) == 2
                ):
                    out = out + "  四库：辰戌丑未"
                print(out)
